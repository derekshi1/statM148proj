...
def build_sequences(exploded: pl.DataFrame) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
    """
    Group by 'id' and collect feature columns into per-journey arrays.
    Also builds a timestamps array (cumulative seconds from journey start)
    used by the time-aware attention mechanism.

    Returns
    -------
    sequences  : dict mapping journey id → (seq_len, num_features) float32 array
    timestamps : dict mapping journey id → (seq_len,) float32 array of cumulative seconds
    """
    sequences  = {}
    timestamps = {}
    grouped = exploded.group_by("id", maintain_order=True)
    ...
    for (id_val,), group in grouped:
        sequences[id_val] = group.select(FEATURE_COLS).to_numpy().astype(np.float32)

        # Cumulative seconds from the first event in this journey.
        # delta_seconds is time since the previous event (0 for the first).
        # cumsum gives absolute elapsed time from journey start per step.
        delta = group.select("delta_seconds").to_numpy().astype(np.float32).flatten()
        timestamps[id_val] = np.cumsum(delta)    # shape: (journey_len,)
...
class JourneyDataset(Dataset):
    ...
    timestamps : dict[id → (seq_len,) np.ndarray]  — cumulative seconds from journey start
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        ts  = self.timestamps[journey_id]         # (raw_len,)
        ...
        if pad_len > 0:
            ...
            ts  = np.concatenate([ts, np.zeros(pad_len, dtype=np.float32)])
        ...
        return {
            "src":          torch.from_numpy(seq),                                   # (T, F)
            "timestamps":   torch.from_numpy(ts),                                    # (T,)
            "padding_mask": padding_mask,                                             # (T,)
            "label":        torch.tensor(self.labels[journey_id], dtype=torch.long), # scalar
            "id":           journey_id,
        }
...
def prepare_datasets(
    ...
    *_labels : dict mapping id string → integer class label
    ...
    # Build sequence and timestamp dicts
    train_seqs, train_ts = build_sequences(train_exp)
    val_seqs,   val_ts   = build_sequences(val_exp)
    test_seqs,  test_ts  = build_sequences(test_exp)
    ...
    # Build Datasets (timestamps are NOT scaled — they stay as raw seconds)
    train_ds = JourneyDataset(train_seqs, train_ts, train_labels, max_seq_len)
    val_ds   = JourneyDataset(val_seqs,   val_ts,   val_labels,   max_seq_len)
    test_ds  = JourneyDataset(test_seqs,  test_ts,  test_labels,  max_seq_len)
...
def train_step(model, batch: dict, optimizer, criterion, device):
    ...
    ts    = batch["timestamps"].to(device)     # (B, T)
    mask  = batch["padding_mask"].to(device)   # (B, T)  True = ignore
    label = batch["label"].to(device)          # (B,)

    logits = model(src, timestamps=ts, src_padding_mask=mask)  # (B, num_classes)
...
def inspect(df: pl.DataFrame, n: int = 3):
    ...
    for i, (k, v) in enumerate(seqs.items()):
        ...
        print(f"  timestamp range     : 0 → {ts[k][-1]:.0f}s ({ts[k][-1]/3600:.1f}h)")
        if i >= n - 1:
            break
